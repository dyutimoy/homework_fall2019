...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        #self.rand_indices=np.random.permutation(self.obs.shape[0]-batch_size)
        #return self.obs[ self.rand_indices:], self.acs[-batch_size:], self.rews[-batch_size:], self.next_obs[-batch_size:], self.terminals[-batch_size:]
        self.rand_indices=np.random.permutation(np.arange(self.obs.shape[0]))
        return self.obs[self.rand_indices][-batch_size:], self.acs[self.rand_indices][-batch_size:], self.rews[self.rand_indices][-batch_size:], self.next_obs[self.rand_indices][-batch_size:], self.terminals[self.rand_indices][-batch_size:]
